...
func printExpandConfigUsage() {
	fmt.Print(`Usage: mkvdup expand-config [options] <config-file>...

Expand a mount config's include globs into an explicit file list.

Reads one or more standard mount config files (the same format accepted by
mount, validate, and reload), resolves all includes glob patterns, and writes
an expanded config with each matched .mkvdup.yaml explicitly listed.

This is useful with the FUSE mount's file watcher: point the mount at the
expanded config, then re-run expand-config when new .mkvdup.yaml files are
added. The mount detects the config change and adds the new virtual files.

Arguments:
    <config-file>  One or more config files to expand (same format as mount)
...
The output uses the same includes format, so it works directly with mount:
    mkvdup mount /mnt/videos expanded-config.yaml

If the --output file already exists and the includes list is unchanged,
the file is not rewritten (avoiding unnecessary mount reloads).
...
Examples:
    ...
    mkvdup expand-config --config-dir /etc/mkvdup/configs/
`)
}
