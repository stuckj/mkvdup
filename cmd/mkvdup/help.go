...
func printExpandConfigUsage() {
	fmt.Print(`Usage: mkvdup expand-config [options] <wildcard-config.yaml>

Expand a wildcard config into an explicit list of .mkvdup.yaml files.

Reads a config file containing glob patterns (sources with path + pattern),
walks the filesystem to resolve all matching .mkvdup.yaml files, and writes
...
Arguments:
    <wildcard-config.yaml>  Config file with glob patterns to expand
...
Input config format:
    sources:
      - path: /data/isos/dvds
        pattern: "**/*.mkvdup.yaml"
      - path: /data/isos/blurays
        pattern: "**/*.mkvdup.yaml"

Output format:
    # Auto-generated by: mkvdup expand-config wildcard-config.yaml
    # Source: /path/to/wildcard-config.yaml
    # Generated: 2026-03-24T12:00:00Z
    # Matched files: 42
    includes:
      - "/data/isos/dvds/movie1/movie1.mkvdup.yaml"
      - "/data/isos/dvds/movie2/movie2.mkvdup.yaml"
      # ... all matched files explicitly listed

The expanded config uses 'includes' format, so it works directly with mount:
    mkvdup mount /mnt/videos expanded-config.yaml

Recommended workflow:
    1. Keep a simple wildcard config as the source of truth
    2. Run 'mkvdup expand-config wildcard.yaml --output expanded.yaml'
    3. Point the FUSE mount at expanded.yaml
    4. When new .mkvdup files are added, re-run expand-config
    5. The mount detects the config change and adds new virtual files

Examples:
    mkvdup expand-config wildcard-config.yaml
    mkvdup expand-config wildcard-config.yaml --output expanded.yaml
    mkvdup expand-config --dry-run wildcard-config.yaml
`)
}
