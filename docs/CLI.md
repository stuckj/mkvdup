...
```bash
mkvdup expand-config <config-file>...
mkvdup expand-config <config-file> --output expanded.yaml
mkvdup expand-config --dry-run <config-file>
...
**Input:** The input is a standard mount config — the same format accepted by `mount`, `validate`, and `reload`:
...
The output is deterministic: files are sorted alphabetically and deduplicated. If `--output` targets an existing file and the includes list is unchanged, the file is not rewritten (avoiding unnecessary mount reloads).
...
# Expand from a directory of configs
mkvdup expand-config --config-dir /etc/mkvdup/configs/
```
