...
| Field | Required | Description |
|-------|----------|-------------|
| `sources` | Yes | List of source entries |
| `sources[].path` | Yes | Base directory to search |
| `sources[].pattern` | Yes | Glob pattern relative to the path (supports `**` for recursive matching) |
...
The output is deterministic: files are sorted alphabetically and deduplicated across sources.

**Use with mount:** The expanded config uses the standard `includes` format, so it works directly with `mkvdup mount` as a config argument.
